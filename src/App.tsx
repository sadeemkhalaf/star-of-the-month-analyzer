...
import {
  IResponses,
  IResponsesFinalResult,
  IResponsesResult,
} from "./main/shared/model";
...
function App() {
  ...
  const checkGivenStars = (responses: IResponses[]) => {
    ...
    const filtered = filteredList.map(
      (row) =>
        ({
          ID: row.ID,
          whom: validateNameFormat(row.whom),
          rating: row.rating,
          feedback: {
            givenBy: row.name,
  ...
  const analyzePulledData = useCallback(
    (sheetdata: IResponses[]) => {
      ...
      if (exclude) {
        filteredData = _lodash.filter(sheetdata, (value) => {
          return (
            reformatNameString(value.name) !==
            reformatNameString(validateNameFormat(value.whom))
          );
        });
  ...
  const getTotalOfStars = () => {
    return exclude
      ? finalist.reduce((a, b) => a + b.rating, 0)
      : sheetData.reduce((a, b) => a + b.rating, 0);
  };
  ...
  useEffect(() => {
    if (result.length > 0) {
      setLoading(true);
      let idx = 0;
      const mappedResults = _lodash.mapValues(
        _lodash.groupBy(result, "whom"),
        (rows) => {
          const totalRating = _lodash.sumBy(rows, "rating");
          const concatenatedFeedback = _lodash
            .map(rows, (item) => item.feedback)
            .filter((item) => !!item);
      ...
      const finalRes = _lodash.map(mappedResults, (value, key) => ({
        whom: key,
        ...value,
      }));
  ...
  return (
    <div className={`flex flex-1 flex-col px-5 xl:px-48 lg:px-32 mb-24`}>
      ...
      <div className="w-full h-full flex justify-between items-start flex-row py-4 border-b-[0.5px] border-b-black bg-white rounded-md p-3">
        <div className="flex-col">
          <div className="pb-4">
            <p className="text-xs py-2 text-gray-500">
              {"import the excel sheet downloaded from responses:"}
            </p>
            <ExcelUpload
              setSheetData={setSheetData}
              analyzePulledData={analyzePulledData}
              loading={false}
              sheetData={sheetData}
              dateRange={dateRange}
            />
        ...
        <div>
          <span>
            <FormLabel title="exclude-stars">
              {"Exclude self given stars"}
            </FormLabel>
            <Switch onChange={excludeResults} color="success" />
          </span>
          <button
            className="bg-transparent font-semibold py-2 px-4 border text-red-600 border-none my-4"
            onClick={() => {
              clearResults();
              setSheetData([]);
            }}
      ...
      {sheetData.length > 0 && (
        <div className="w-full h-full flex justify-around items-center flex-row py-4 border-b-[0.5px] border-b-black bg-white rounded-md p-3 my-3">
          <div className="text-sm">
            Total stars:{" "}
            <span className="text-lg font-bold">{`${getTotalOfStars()}/`}</span>
            <span className="text-sm font-normal">{`${
              getTotalOfPeopleVoted() * 5
            }`}</span>
          </div>
          <div className="text-sm">
            Total people voted:{" "}
            <span className="text-lg font-bold">{`${getTotalOfPeopleVoted()}`}</span>
          </div>
        </div>
      )}
